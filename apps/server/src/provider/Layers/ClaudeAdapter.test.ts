...
function makeHarness(config?: {
  ...
  return {
    layer: makeClaudeAdapterLive(adapterOptions).pipe(
      Layer.provideMerge(
        Layer.succeed(ExecutionTargetRuntime, {
          startProviderSession: (input, handlers) =>
            Effect.tryPromise(() => handlers.startLocal(input)),
        }),
      ),
      Layer.provideMerge(
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("configures an SSH spawn wrapper for remote Claude sessions", () => {
    const harness = makeHarness();
    let capturedSpawnFactory:
      | ((
          options: Parameters<NonNullable<ClaudeQueryOptions["spawnClaudeCodeProcess"]>>[0],
        ) => unknown)
      | undefined;

    const layer = makeClaudeAdapterLive({
      createQuery: (input) => {
        capturedSpawnFactory = input.options.spawnClaudeCodeProcess;
      ...
      Layer.provideMerge(
        Layer.succeed(ExecutionTargetRuntime, {
          startProviderSession: (_input, handlers) =>
            Effect.tryPromise(() =>
              handlers.startSsh(
                {
                  threadId: THREAD_ID,
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(typeof capturedSpawnFactory, "function");
      assert.equal(session.targetId, "ssh-target");
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
