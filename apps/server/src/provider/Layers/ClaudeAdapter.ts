...
import { createClaudeSshSpawnProcess } from "./ClaudeExecutionTarget.ts";
...
export interface ClaudeAdapterLiveOptions {
  ...
  readonly createSshSpawnProcess?: (input: {
    readonly runtimeInput: ProviderSessionStartInput;
    readonly ssh: {
      readonly kind: "ssh";
...
function makeClaudeAdapter(options?: ClaudeAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const createSshSpawnProcess =
      options?.createSshSpawnProcess ??
      ((input) => createClaudeSshSpawnProcess(input.runtimeInput, input.ssh));
        ...
        const prompt: AsyncIterable<SDKUserMessage> = {
          [Symbol.asyncIterator]() {
            return {
              next: async () =>
                Effect.runPromise(
                  Queue.take(promptQueue).pipe(
                    Effect.map((item) =>
                      item.type === "message"
                        ? { done: false, value: item.message }
                        : ({ done: true, value: undefined } as IteratorResult<SDKUserMessage>),
                    ),
                  ),
                ),
            };
    ...
    const startSession: ClaudeAdapterShape["startSession"] = (input) => {
      if (input.provider !== undefined && input.provider !== PROVIDER) {
        return startSessionInternal(input);
      }

      return executionTargetRuntime
        .startProviderSession(input, {
          startLocal: (runtimeInput) => Effect.runPromise(startSessionInternal(runtimeInput)),
          startSsh: (runtimeInput, ssh) =>
            Effect.runPromise(
              startSessionInternal(
                runtimeInput,
                createSshSpawnProcess({
                  runtimeInput,
                  ssh,
                }),
              ),
