...
interface CheckpointStoreShapeOptions {
  readonly tempIndexDir?: string;
  readonly cleanupTempIndex?: boolean;
}

export function makeCheckpointStoreShape(options: CheckpointStoreShapeOptions = {}) {
  return Effect.gen(function* () {
    ...
    const captureCheckpoint: CheckpointStoreShape["captureCheckpoint"] = (input) =>
      Effect.gen(function* () {
        if (options.cleanupTempIndex === false) {
          const tempIndexPath = path.join(
            options.tempIndexDir ?? "/tmp",
            `t3-fs-checkpoint-index-${randomUUID()}`,
          );
          yield* runCaptureCheckpoint(input, tempIndexPath);
          return;
        }

        yield* Effect.acquireUseRelease(
          fs.makeTempDirectory({ prefix: "t3-fs-checkpoint-" }),
          (tempDir) => runCaptureCheckpoint(input, path.join(tempDir, `index-${randomUUID()}`)),
