...
export const ProjectColorPicker = memo(function ProjectColorPicker({
  ...
  const currentInputColor = normalizeColorInputValue(resolvedColor);
  const isPresetColor =
    resolvedColor !== null &&
    PROJECT_COLOR_PRESETS.some(
      (preset) => preset.value.toLowerCase() === resolvedColor.toLowerCase(),
    );
  ...
  const handleClear = useCallback(
    (event: React.MouseEvent) => {
      ...
      void updateProjectColor(projectId, null);
      setOpen(false);
    },
    [projectId],
  );
  ...
  const handleCustomColorChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      void updateProjectColor(projectId, nextColor);
    },
    [projectId],
  );
  ...
  return (
    <>
      <button
        ...
        className="flex size-6 shrink-0 items-center justify-center rounded-md p-0 text-muted-foreground/70 hover:bg-secondary hover:text-foreground sm:size-5"
        title="Set project color"
        aria-label="Set project color"
      >
        {resolvedColor ? (
          <span className="size-2.5 rounded-full" style={{ backgroundColor: resolvedColor }} />
        ) : (
        ...
        >
          <div className="grid grid-cols-5 gap-1.5 sm:grid-cols-6">
            {PROJECT_COLOR_PRESETS.map((preset) => (
              <button
                key={preset.value}
                type="button"
                onClick={(e) => handleSelectColor(e, preset.value)}
                className={`size-7 rounded-md border-2 transition-all hover:scale-110 active:scale-95 sm:size-6 ${
                  resolvedColor?.toLowerCase() === preset.value.toLowerCase()
          ...
          <div className="mt-2 rounded-md border border-border/70 bg-background/70 p-2">
            <div className="mb-1 flex items-center justify-between gap-2">
              ...
              {resolvedColor && !isPresetColor ? (
                <span className="text-[10px] text-muted-foreground">{resolvedColor}</span>
              ) : null}
            </div>
            <input
              id={colorInputId}
